from typing import List
from pydantic import BaseModel

from py_ocpi.core.data_types import CiString, URL, String
from py_ocpi.core.enums import RoleEnum
from py_ocpi.modules.locations.v_2_3_0.schemas import BusinessDetails


class CredentialsRole(BaseModel):
    """
    https://github.com/ocpi/ocpi/blob/2.3.0/credentials.asciidoc#141-credentialsrole-class
    """

    role: RoleEnum
    business_details: BusinessDetails
    party_id: CiString(3)  # type: ignore
    country_code: CiString(2)  # type: ignore


class Credentials(BaseModel):
    """
    https://github.com/ocpi/ocpi/blob/2.3.0/credentials.asciidoc#131-credentials-object
    """

    token: String(64)  # type: ignore
    url: URL
    roles: List[CredentialsRole]
