...
class AuthMethod(str, Enum):
    """
    https://github.com/ocpi/ocpi/blob/2.3.0/mod_cdrs.asciidoc#141-authmethod-enum
    """
...
class CdrDimensionType(str, Enum):
    """
    https://github.com/ocpi/ocpi/blob/2.3.0/mod_cdrs.asciidoc#143-cdrdimensiontype-enum
    """

    # Average charging current during this ChargingPeriod: defined in A
    # (Ampere).
    # When negative, the current is flowing from the EV to the grid.
    current = "CURRENT"
    # Total amount of energy (dis-)charged during this ChargingPeriod:
    # defined in kWh.
    # When negative, more energy was feed into the grid then
    # charged into the EV.
    ...
    # ChargingPeriod, when negative,
    # current has flowed from the EV to the grid. Defined in A (Ampere).
    min_current = "MIN_CURRENT"
    # Maximum power reached during this ChargingPeriod: defined in kW
    # (Kilowatt).
    max_power = "MAX_POWER"
    # Minimum power reached during this ChargingPeriod: defined in kW
    # (Kilowatt),
