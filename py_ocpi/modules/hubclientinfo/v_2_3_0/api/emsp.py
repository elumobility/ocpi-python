...
from py_ocpi.modules.hubclientinfo.v_2_3_0.schemas import ClientInfo
...
async def get_hubclientinfo(
    ...
    **Raises:**
    ...
    if data:
        return OCPIResponse(
            data=[
                adapter.hubclientinfo_adapter(data, VersionNumber.v_2_3_0).model_dump()
            ],
...
async def add_or_update_clienthubinfo(
    ...
    **Returns:**
    ...
    return OCPIResponse(
        data=[adapter.hubclientinfo_adapter(data, VersionNumber.v_2_3_0).model_dump()],
        **status.OCPI_1000_GENERIC_SUCESS_CODE,
    )
