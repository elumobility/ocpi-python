...
async def get_location(
    ...
    **Raises:**
    ...
    if data:
        return OCPIResponse(
            data=[adapter.location_adapter(data, VersionNumber.v_2_2_1).model_dump()],
...
async def get_evse(
    ...
    **Raises:**
    ...
    if data:
        location = adapter.location_adapter(data, VersionNumber.v_2_2_1)
...
async def add_or_update_evse(
    ...
    **Raises:**
    ...
    if old_data:
        old_location = adapter.location_adapter(old_data, VersionNumber.v_2_2_1)
    ...
    if old_data:
        ...
        for old_evse in old_location.evses:
            if old_evse.uid == evse_uid:
                ...
                try:
                    location_dict = new_location.model_dump()
                except Exception as e:
                    logger.error(f"Error converting location to dict: {type(e).__name__}: {e}")
                    raise
                ...
                try:
                    evse_dict = new_evse.model_dump()
                except Exception as e:
                    logger.error(f"Error converting evse to dict: {type(e).__name__}: {e}")
                    raise
